...
public class MftReader
{
    ...
    private IEnumerable<UsnRecord> EnumerateFiles(SafeFileHandle volumeRootHandle)
    {
        //var rootFrn = GetRootFrnEntry(drive);
        var symLinks = new List<UsnRecord>();
        ...
        try
        {
            while (Win32.DeviceIoControl(volumeRootHandle, PInvoke.FSCTL_ENUM_USN_DATA, medBuffer,
                medBuffer.Size, pData, sizeOfBuffer, out uint outBytesReturned, IntPtr.Zero))
            {
    ...
    private void HandleSymLinks(SafeFileHandle volumeRootHandle, IEnumerable<UsnRecord> symLinks)
    {
    ...
    private USN_JOURNAL_DATA_V2 SetupMFT_Enum_DataBuffer(SafeFileHandle changeJournalRootHandle)
    {
        var sizeMftEnumData = Marshal.SizeOf(typeof(USN_JOURNAL_DATA_V2));

        if (Win32.DeviceIoControl(changeJournalRootHandle,
            PInvoke.FSCTL_QUERY_USN_JOURNAL,   // IO Control Code  
            IntPtr.Zero,                // In Buffer  
            0,                          // In Buffer Size  
            out USN_JOURNAL_DATA_V2 ujd,                  // Out Buffer  
